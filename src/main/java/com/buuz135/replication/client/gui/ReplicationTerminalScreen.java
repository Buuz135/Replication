...
public class ReplicationTerminalScreen extends AbstractContainerScreen<ReplicationTerminalContainer> {
    ...
    protected void containerTick() {
        ...
        for (MatterPatternButton matterPatternButton : this.patternMenu.matterPatternButtonList) {
            if (matterPatternButton.cachedAmount() == -1 && (matterPatternButton.createdWhen() + 3*20) < Minecraft.getInstance().level.getGameTime()){
    ...
    public boolean mouseClicked(double p_98531_, double p_98532_, int p_98533_) {
        if (p_98533_ == 0) {
            if (this.insideScrollbar(p_98531_, p_98532_)) {
                ...
                int i = this.topPos + 18;
                int j = i + 80;
                this.scrollOffs = ((float) p_98532_ - (float) i - 7.5F) / ((float) (j - i) - 15.0F);
                this.scrollOffs = Mth.clamp(this.scrollOffs, 0.0F, 1.0F);
                this.patternMenu.scrollTo(this.scrollOffs);
                return true;
            }
